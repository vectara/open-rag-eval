...
class VectaraConnector(Connector):
    ...
    def fetch_data(self):
        ...
        if not self.parallel:
            with open(answers_path, "w", newline="",
                ...
                for query in tqdm(queries,
                    ...
                    for run_idx in range(self.repeat_query):
                        # Pass run_idx+1 directly to process_query
                        results = self.process_query(query, endpoint_url, headers,
                                                     self.query_config, run_idx + 1)
                        if results:
                            for row in results:
                                writer.writerow(row)
        else:
            ...
            for query in queries:
                for run_idx in range(self.repeat_query):
                    # Store both the query and which run it is
                    repeated_queries.append((query, run_idx + 1))

            results_buffer = []
            with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
                future_to_query_info = {
                    executor.submit(self.process_query, query, endpoint_url, headers, self.query_config, run_idx):
    ...
    def process_query(self, query, endpoint_url, headers, query_config, run_idx):
        """
