...
class ROUGEAnswerSimilarityMetric(PairwiseAnswerSimilarityMetric):
    ...
    def compute(self, multi_rag_result: MultiRAGResult) -> List[float]:
        ...
        try:
            ...
            for i, j in itertools.combinations(range(len(answers)), 2):
                # Skip empty answers
                if not answers[i].strip() or not answers[j].strip():
                    continue

                scores = rouge.get_scores(answers[i], answers[j])[0]
                # Use ROUGE-L F1 as the similarity metric
            ...
            logging.exception(f"Error in ROUGE similarity calculation: {str(e)}")
            return []